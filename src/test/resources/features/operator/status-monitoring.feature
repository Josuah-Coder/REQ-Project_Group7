Feature: Status Monitoring
  ...
  @Operator @MVP2
  Scenario: Filter dashboard options
    Given I open the status dashboard
    And the network has 25 locations with 180 charging points
    When I apply the following filters:
      | Filter               | Value                    |
      | Region               | "Berlin"                 |
      | Availability         | "< 70%"                  |
      | Charger Type         | "DC"                     |
      | Last Hour            | "Activity > 0"           |
      | Maintenance Status   | "No Maintenance"         |
    Then I see 3 locations in filtered results:
      | Location             | DC Points | Available | Last Activity   |
      | Berlin-Alex          | 4/6       | 67%       | 15 minutes ago  |
      | Berlin-Mitte         | 2/3       | 66%       | 8 minutes ago   |
      | Berlin-West          | 3/5       | 60%       | 25 minutes ago  |
    And the overview shows:
      | Metric               | Value     |
      | Filtered Locations   | 3         |
      | Total DC Points      | 14        |
      | DC Availability      | 64%       |
      | Active Sessions      | 7         |